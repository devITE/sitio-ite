const Normatividad__pdflinkItem = require.context("../pdf/normatividad");

export const dataNormatividad = [
  {
    id: 1,
    nameButton: "lFederales",
    titleButton: "Leyes Federales",
    children: [
      {
        id: 1,
        titleItem: "Constitución Política de los Estados Unidos Mexicanos",
        typeDocument: "pdf",
        linkItem: Normatividad__pdflinkItem(`./${"lFederales/"}${"1.pdf"}`),
      },
      {
        id: 2,
        titleItem: "Ley Federal de Consulta Popular",
        typeDocument: "pdf",
        linkItem: Normatividad__pdflinkItem(`./${"lFederales/"}${"2.pdf"}`),
      },
      {
        id: 3,
        titleItem: "Ley General de Instituciones y Procedimientos Electorales",
        typeDocument: "pdf",
        linkItem: Normatividad__pdflinkItem(`./${"lFederales/"}${"3.pdf"}`),
      },
      {
        id: 4,
        titleItem: "Ley General de Partidos Políticos",
        typeDocument: "pdf",
        linkItem: Normatividad__pdflinkItem(`./${"lFederales/"}${"4.pdf"}`),
      },
      {
        id: 5,
        titleItem:
          "Ley General de Sistema de Medios de Impugnación en Materia Electoral",
        typeDocument: "pdf",
        linkItem: Normatividad__pdflinkItem(`./${"lFederales/"}${"5.pdf"}`),
      },
      {
        id: 6,
        titleItem: "Ley General de Transparencia y Acceso a la Información",
        typeDocument: "pdf",
        linkItem: Normatividad__pdflinkItem(`./${"lFederales/"}${"6.pdf"}`),
      },
      {
        id: 7,
        titleItem: "Ley General en Materia de Delitos Electorales",
        typeDocument: "pdf",
        linkItem: Normatividad__pdflinkItem(`./${"lFederales/"}${"7.pdf"}`),
      },
      {
        id: 8,
        titleItem: "Ley General de Archivo",
        typeDocument: "pdf",
        linkItem: Normatividad__pdflinkItem(`./${"lFederales/"}${"8.pdf"}`),
      },
      {
        id: 9,
        titleItem: "LLey General del Sistema Nacional Anticorrupción",
        typeDocument: "pdf",
        linkItem: Normatividad__pdflinkItem(`./${"lFederales/"}${"9.pdf"}`),
      },
      {
        id: 10,
        titleItem:
          "Ley General de Protección de Datos Personales en Posesión de Sujetos Obligados",
        typeDocument: "pdf",
        linkItem: Normatividad__pdflinkItem(`./${"lFederales/"}${"10.pdf"}`),
      },
      {
        id: 11,
        titleItem: "Ley General de Contabilidad Gubernamental",
        typeDocument: "pdf",
        linkItem: Normatividad__pdflinkItem(`./${"lFederales/"}${"11.pdf"}`),
      },
    ],
  },
  {
    id: 2,
    nameButton: "lEstatales",
    titleButton: "Leyes Estatales",
    children: [
      {
        id: 1,
        titleItem: "Constitución Política del Estado de Tlaxcala",
        typeDocument: "pdf",
        linkItem: Normatividad__pdflinkItem(`./${"lEstatales/"}${"1.pdf"}`),
      },
      {
        id: 2,
        titleItem:
          "Ley de transparencia y Acceso a la Información Pública del Estado de Tlaxcala",
        typeDocument: "pdf",
        linkItem: Normatividad__pdflinkItem(`./${"lEstatales/"}${"2.pdf"}`),
      },
      {
        id: 3,
        titleItem: "Ley de Archivos del Estado de Tlaxcala",
        typeDocument: "pdf",
        linkItem: Normatividad__pdflinkItem(`./${"lEstatales/"}${"3.pdf"}`),
      },
      {
        id: 4,
        titleItem: "Ley de Consulta Ciudadana para el Estado de Tlaxcala",
        typeDocument: "pdf",
        linkItem: Normatividad__pdflinkItem(`./${"lEstatales/"}${"4.pdf"}`),
      },
      {
        id: 5,
        titleItem:
          "Ley de Instituciones y Procedimientos Electorales para el Estado de Tlaxcala",
        typeDocument: "pdf",
        linkItem: Normatividad__pdflinkItem(`./${"lEstatales/"}${"5.pdf"}`),
      },
      {
        id: 6,
        titleItem:
          "Ley de Medios de Impugnación en Materia Electoral para el Estado de Tlaxcala",
        typeDocument: "pdf",
        linkItem: Normatividad__pdflinkItem(`./${"lEstatales/"}${"6.pdf"}`),
      },
      {
        id: 7,
        titleItem: "Ley de Partidos Políticos para el Estado de Tlaxcala",
        typeDocument: "pdf",
        linkItem: Normatividad__pdflinkItem(`./${"lEstatales/"}${"7.pdf"}`),
      },
      {
        id: 8,
        titleItem:
          "Ley de Protección de Datos Personales en posesion de sujetos obligados para el Estado de Tlaxcala",
        typeDocument: "pdf",
        linkItem: Normatividad__pdflinkItem(`./${"lEstatales/"}${"8.pdf"}`),
      },
      {
        id: 9,
        titleItem:
          "Ley de Responsabilidades de los Servidores Públicos del Estado de Tlaxcala",
        typeDocument: "pdf",
        linkItem: Normatividad__pdflinkItem(`./${"lEstatales/"}${"9.pdf"}`),
      },
      {
        id: 10,
        titleItem: "Ley del sistema anticorrupcion del estado de tlaxcala",
        typeDocument: "pdf",
        linkItem: Normatividad__pdflinkItem(`./${"lEstatales/"}${"10.pdf"}`),
      },
      {
        id: 11,
        titleItem:
          "Ley Laboral de los Servidores Públicos del Estado de Tlaxcala y sus Municipios",
        typeDocument: "pdf",
        linkItem: Normatividad__pdflinkItem(`./${"lEstatales/"}${"11.pdf"}`),
      },
      {
        id: 12,
        titleItem:
          "Ley de Fiscalización Superior para el Estado de Tlaxcala y sus Municipios",
        typeDocument: "pdf",
        linkItem: Normatividad__pdflinkItem(`./${"lEstatales/"}${"12.pdf"}`),
      },
      {
        id: 13,
        titleItem:
          "Ley de Procedimiento Administrativo del Estado de Tlaxcala y sus Municipios",
        typeDocument: "pdf",
        linkItem: Normatividad__pdflinkItem(`./${"lEstatales/"}${"13.pdf"}`),
      },
      {
        id: 14,
        titleItem: "Ley Municipal del Estado de Tlaxcala",
        typeDocument: "pdf",
        linkItem: Normatividad__pdflinkItem(`./${"lEstatales/"}${"14.pdf"}`),
      },
      {
        id: 15,
        titleItem:
          "Ley de Entrega  Recepción para el Estado de Tlaxcala y sus Municipios",
        typeDocument: "pdf",
        linkItem: Normatividad__pdflinkItem(`./${"lEstatales/"}${"15.pdf"}`),
      },
    ],
  },
  {
    id: 3,
    nameButton: "reglamentos",
    titleButton: "Reglamentos",
    children: [
      {
        id: 1,
        titleItem:
          "Reglamento del Instituto Tlaxcalteca de Elecciones en Materia de Transparencia y Acceso a la Información Pública",
        typeDocument: "pdf",
        linkItem: Normatividad__pdflinkItem(`./${"reglamentos/"}${"1.pdf"}`),
      },
      {
        id: 2,
        titleItem:
          "Reglamento de Quejas y Denuncias del Instituto Tlaxcalteca de Elecciones",
        typeDocument: "pdf",
        linkItem: Normatividad__pdflinkItem(`./${"reglamentos/"}${"2.pdf"}`),
      },
      {
        id: 3,
        titleItem:
          "Reglamento de Asistencia Técnica, Jurídica y Logística a las Comunidades que realizan elecciones de Presidentes de Comunidad por el Sistema de Usos y Costumbres",
        typeDocument: "pdf",
        linkItem: Normatividad__pdflinkItem(`./${"reglamentos/"}${"3.pdf"}`),
      },
      {
        id: 4,
        titleItem:
          "Reglamento de la Oficialía Electoral del Instituto Tlaxcalteca de Elecciones",
        typeDocument: "pdf",
        linkItem: Normatividad__pdflinkItem(`./${"reglamentos/"}${"4.pdf"}`),
      },
      {
        id: 5,
        titleItem:
          "Reglamento Interior del Instituto Tlaxcalteca de Elecciones",
        typeDocument: "pdf",
        linkItem: Normatividad__pdflinkItem(`./${"reglamentos/"}${"5.pdf"}`),
      },
      {
        id: 6,
        titleItem:
          "Reglamento de los Consejos Distritales y Municipales Electorales del Instituto Tlaxcalteca de Elecciones",
        typeDocument: "pdf",
        linkItem: Normatividad__pdflinkItem(`./${"reglamentos/"}${"6.pdf"}`),
      },
      {
        id: 7,
        titleItem:
          "Reglamento para la Clasificación y Desclasificación de la Información del Instituto Tlaxcalteca de Elecciones",
        typeDocument: "pdf",
        linkItem: Normatividad__pdflinkItem(`./${"reglamentos/"}${"7.pdf"}`),
      },
      {
        id: 8,
        titleItem:
          "Reglamento registro cancelación de la acreditación activos de los partidos políticos",
        typeDocument: "pdf",
        linkItem: Normatividad__pdflinkItem(`./${"reglamentos/"}${"8.pdf"}`),
      },
      {
        id: 9,
        titleItem:
          "Reglamento de Comisiones del Instituto Tlaxcalteca de Elecciones",
        typeDocument: "pdf",
        linkItem: Normatividad__pdflinkItem(`./${"reglamentos/"}${"9.pdf"}`),
      },
      {
        id: 10,
        titleItem: "Reglamento de sesiones del Consejo General",
        typeDocument: "pdf",
        linkItem: Normatividad__pdflinkItem(`./${"reglamentos/"}${"10.pdf"}`),
      },
      {
        id: 11,
        titleItem:
          "Reglamento del Comité de Igualdad Laboral y No Discriminación del Instituto Tlaxcalteca de Elecciones",
        typeDocument: "pdf",
        linkItem: Normatividad__pdflinkItem(`./${"reglamentos/"}${"11.pdf"}`),
      },
      {
        id: 12,
        titleItem:
          "Reglamento de la Junta General Ejecutiva del Instituto Tlaxcalteca de Elecciones",
        typeDocument: "pdf",
        linkItem: Normatividad__pdflinkItem(`./${"reglamentos/"}${"12.pdf"}`),
      },
    ],
  },
  {
    id: 4,
    nameButton: "compendio",
    titleButton: "Compendio Legislativo Electoral",
    children: [
      {
        id: 1,
        titleItem: "Portada",
        typeDocument: "pdf",
        linkItem: Normatividad__pdflinkItem(`./${"compendio/"}${"1.pdf"}`),
      },
      {
        id: 2,
        titleItem: "Constitución Local",
        typeDocument: "pdf",
        linkItem: Normatividad__pdflinkItem(`./${"compendio/"}${"2.pdf"}`),
      },
      {
        id: 3,
        titleItem: "LIPEET",
        typeDocument: "pdf",
        linkItem: Normatividad__pdflinkItem(`./${"compendio/"}${"3.pdf"}`),
      },
      {
        id: 4,
        titleItem: "LPPET",
        typeDocument: "pdf",
        linkItem: Normatividad__pdflinkItem(`./${"compendio/"}${"4.pdf"}`),
      },
      {
        id: 5,
        titleItem: "LMIMEET",
        typeDocument: "pdf",
        linkItem: Normatividad__pdflinkItem(`./${"compendio/"}${"5.pdf"}`),
      },
      {
        id: 6,
        titleItem: "Delitos Electorales",
        typeDocument: "pdf",
        linkItem: Normatividad__pdflinkItem(`./${"compendio/"}${"6.pdf"}`),
      },
      {
        id: 7,
        titleItem: "Reglamentos y Lineamientos",
        typeDocument: "pdf",
        linkItem: Normatividad__pdflinkItem(`./${"compendio/"}${"7.pdf"}`),
      },
    ],
  },
];
