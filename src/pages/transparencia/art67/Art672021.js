...
const renderTransparencia = ({ row }) => {
  const { excel1, excel2, excel3, letra } = row.original;
  const files = [excel1, excel2, excel3];
...
const Art672021 = () => {
  useEffect(() => {
    document.title = `Artículo 67 2021`;
  }, []);
  ...
  return (
    <>
      <TitlePages
        title="Transparencia"
        subTitle="Artículo 67. (2021) Obligaciones Específicas"
      />
      <Expandible />
